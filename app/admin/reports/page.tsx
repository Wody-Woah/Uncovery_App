...
export default function AdminReportsPage() {
  ...
  async function fetchReports(status: 'pending' | 'resolved', pageNum: number) {
    setFetching(true)
    ...
    const { data: reportsData, count } = await supabase
      .from('group_reports')
      .select('id, group_id, reporter_user_id, reported_user_id, message_content, reason, reason_note, status, created_at', { count: 'exact' })
      .eq('status', status)
    ...
    if (!reportsData?.length) { setReports([]); setFetching(false); return }
  ...
  async function handleFilterChange(f: 'pending' | 'resolved') {
    setFilter(f)
    setPage(0)
    await fetchReports(f, 0)
  }

  async function handlePageChange(newPage: number) {
    setPage(newPage)
    await fetchReports(filter, newPage)
  ...
  async function handleDismiss(reportId: string) {
    ...
    if (filter === 'pending') setPendingCount((prev) => prev - 1)
    setActioning(null)
  }
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-2">
              <button
                ...
                className="rounded-lg border border-steel/20 bg-white px-4 py-2 text-xs font-medium text-charcoal hover:bg-canvas transition-colors disabled:opacity-40"
              >
                ← Previous
              </button>
              <p className="text-xs text-white/70 text-shadow-hero">
